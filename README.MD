...
| pattern | `NewThrottlerPattern(patterns ...Pattern)` | Throttles if matching throttler from provided patterns throttle.<br> Use `WithKey(ctx context.Context, key string) context.Context` to specify key for regex pattern throttler matching.|
